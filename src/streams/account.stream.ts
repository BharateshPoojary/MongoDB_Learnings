...
const startAccountChangeStream = () => {
  const pipleine: Array<Record<string, unknown>> = [
    {
      $match: {
        operationType: "update", //By specifying the operation type it allow us to filter the action which is performed on collection at database level or else we have to filter the event which is required by us at teh callback of the on change of the collection
      },
    },
  ];
  const options: ChangeStreamOptions = {
    //$changeStream stage  only works when you are using mongodb Driver directly , for mongoose we have to pass it as options in watch method
